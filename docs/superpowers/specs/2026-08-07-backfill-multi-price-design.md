...
| 復活通知 | **復活當下若場次尚未開始（`start_at > now`）→ 補發一則「課程成班」通知給帶課教練**（沿用既有 `course_confirmed_coach` 模板；count＝復活後該場 confirmed 人數）。過去場次復活照舊靜默；學員照舊不發。 |
...
2. **選填 `price`**：新參數 `price = null`。驗證：非 null 時必須為非負整數（`Number.isInteger(price) && price >= 0`），否則 `400 invalid_price`；null 則沿用 `tpl.price_per_session`。後續所有用價處（paid 直開已核對單的金額、未付併單的加總、`insertReg` 的 `price_at_registration`）一律用解析後的價。
...
- 單元測試（`tests/admin-group-reg.test.js` 延伸）：
  ...
  - `price` 覆寫：訂單金額與 `price_at_registration` 用覆寫值；`price=0` 可；未帶 `price` 沿用範本價；`price` 非法（負數/小數/字串）→ 400 `invalid_price`。
